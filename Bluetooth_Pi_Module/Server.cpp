					...
					.gattCharacteristicEnd()
							...
							// We can handle updates in any way we wish, but the most common use is to send a change notification.
									...
									.gattCharacteristicEnd()
											...
											/*
												This will need to have a call to allow a connection of a new device.
												This change will need to have the proper gat service begin when the event occurs.
												0x2ABE
												https://btprodspecificationrefs.blob.core.windows.net/assigned-numbers/Assigned%20Number%20Types/Assigned_Numbers.pdf
											*/
											//0x1800 Generic Access Service
											//
											.gattServiceBegin("connect", "1800")
											// Appearance: 0x2A01
											//device name 0x2ABE
											.gattCharacteristicbegin("name", "2ABE", { "read","write" })
											//this will allow us to read in the appropraite value on runtime
											.onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA{
												std::string name = self.methodReturnVariant(pInvocation, ServerUtils::getConnectedName());
												return name;
												})
											//when the name/new is called it should grab the name string object using the std::string, if it doesnt work needs to be a pointer that is 
													//processed
													.onWriteValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA{
															std::string name = self.getDataValue<std::string>("name/new");
