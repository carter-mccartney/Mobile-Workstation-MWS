...
namespace ggk {
	...
	struct ServerUtils
	{
		...
		//this is to set the ConnectedName on each setUp so we know what is connected
		static void setConnectedName(std::string name);

		static std::string getConnectedName();

	private:
		//return the connected name
		std::string connectedName;

		//need to have a timer to count down to display change the name
		//
	};
