...
{
	// Get name.
	std::ifstream nameFile("/home/developer/.BLE_advertising_name");
	std::string shortName;
	std::getline(nameFile, shortName);
	nameFile.close();
	...
	memcpy(&request.data[2], globalUuid, ADVERTISING_UUID_LEN);
...
{
	struct RRequest : HciAdapter::HciHeader
	{
		uint8_t  instance;
	} __attribute__((packed));

	RRequest request;
	request.code = ERemoveAdvertisingCommand;
	request.controllerId = controllerIndex;
	request.dataSize = sizeof(RRequest) - sizeof(HciAdapter::HciHeader);
	request.instance = (unsigned char)0;
	
	if (!HciAdapter::getInstance().sendCommand(request))
	{
		Logger::warn(SSTR << " + Failed to stop advertising with UUID");
		return false;
	}
