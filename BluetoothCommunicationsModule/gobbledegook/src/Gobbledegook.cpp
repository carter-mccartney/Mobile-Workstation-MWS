...
//
int ggkStart(const char *pServiceName, GGKServerDataGetter getter, GGKServerDataSetter setter, int maxAsyncInitTimeoutMS)
{
	try
	{
		//
		...
		Logger::info(SSTR << "Starting GGK server '" << "'");

		// Allocate our server
		TheServer = std::make_shared<Server>(pServiceName, "", "", getter, setter);
